...
@router.post("/results/", response_model=TestResultInDB, status_code=status.HTTP_201_CREATED)
async def create_test_result(
    test_result_data: TestResultCreate,
    session: AsyncSession = Depends(get_session),
    current_user: User = Depends(get_current_user)
):
    created_test_result = await qr.create_test_result(session, current_user.id, test_result_data)
